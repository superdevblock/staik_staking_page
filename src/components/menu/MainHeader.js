...
import { TOKEN_NAME, CONTRACT_ADDRESS, LANGDING_URL, config } from "../../core/config";
...
const Header = function ({ navSelected, setIsOpen }) {
  ...
  return (
    <div className='text-gray-100 flex justify-between' style={{ height: '100px', backgroundColor: '#010101', width: '100%', position: 'fixed', zIndex: '100' }}>
      <div className='flex justify-between z-index-9 p-3'>
        <a href={ LANGDING_URL } className="flex pt-1 pl-1 cursor-pointer hover:text-white">
          <img src="/img/logo.png" style={{height: '60px', paddingRight: '18px'}} /> 
          <span className=" text-3xl pt-2 ">{ TOKEN_NAME }</span>
        </a>
      </div>
      {navSelected !== 'Home' && (
                      ...
                      <div className="flex">
                        <div className="connect-wal ">
                          <div>
                            {/* <img alt='' className='w-6 text-white mr-2' src={WalletIcon}   /> */}
                            <div>
                              <p>{userWalletState && (userWalletState.slice(0, 4) + "..." + userWalletState.slice(38))}</p>
                            </div>
                            <div>
                              <button className="btn-disconnect fs-12" onClick={onDisconnect} style={{ marginTop: "-15px" }}>Disconnect</button>
                            </div>
                          </div>
                        </div>
                      </div>
